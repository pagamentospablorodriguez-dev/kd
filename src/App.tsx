...
function App() {
  ...
  const handleFirstMessage = () => {
    ...
    if (!child) {
      if (children.length === 0) setAppState('setup');
      else setAppState('child_selector');
      return;
    }
    setAppState('chat');
  };
  ...
  // 🔹 Atualizado: redireciona direto para o filho recém-criado
  const handleSetupComplete = async (setupData: ChildSetupData) => {
    ...
    if (!error && newChild) {
      setChild(newChild);
      localStorage.setItem('lastSelectedChild', newChild.id);
      setAppState('chat');
    } else {
  ...
  const handleBackToSelector = () => setAppState('child_selector');
  ...
  const handleMessageLimit = () => setShowLimit(true);

  const handleBackFromSetup = () => {
    if (children.length > 0) setAppState('child_selector');
  ...
  const getBackgroundClass = () => {
    switch (colorScheme) {
      case 'pink': return 'from-white via-pink-50/30 to-rose-50/20';
      case 'blue': return 'from-white via-blue-50/30 to-cyan-50/20';
      default: return 'from-white via-purple-50/30 to-pink-50/20';
    }
  };
